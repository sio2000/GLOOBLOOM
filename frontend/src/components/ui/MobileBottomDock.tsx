...
export function MobileBottomDock() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-auto flex flex-col gap-2 rounded-t-2xl border border-white/8 bg-black/80 backdrop-blur-xl px-3 pt-2 pb-2 shadow-[0_-8px_40px_rgba(0,0,0,0.5)]">
        <button
          type="button"
          onClick={() => setExtrasOpen((v) => !v)}
          className="w-full py-1.5 text-[9px] uppercase tracking-widest text-white/35 hover:text-white/55 transition-colors"
        >
          {extrasOpen ? "Hide feed & comments ▾" : "Live feed & comments ▴"}
        </button>
